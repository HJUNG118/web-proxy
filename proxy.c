...
void modify_HTTP_header(char *http_header, char *method, char *host, char *port, char *path, int server_fd);
...
void doit(int fd) {
  ...
  Rio_readinitb(&client_rio, fd); // 동기화
  ...
  sprintf(from_client_buf, "%s %s %s", method, path, "HTTP/1.0");
  server_fd = Open_clientfd(host, port);

  Rio_readinitb(&server_rio, server_fd);
  modify_HTTP_header(server_buf, method, host, port, path, server_fd);
  ssize_t n;
  while ((n = Rio_readlineb(&server_rio, server_buf, MAXLINE)) > 0)
  {
      Rio_writen(fd, server_buf, n); // 실제 읽은 바이트 수(n)만큼만 쓰도록 수정
  }
  Close(server_fd);
}
